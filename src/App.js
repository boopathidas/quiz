...
function QuizApp() {
  ...
  const [showAnswers, setShowAnswers] = useState(false);  // <-- This line was missing
...
const hideanswer =() =>{
  setShowAnswers(false);  // Hide the answers and show the result page
  setShowResult(true);  // Ensure the result page is shown
};
  const toggleViewAnswers = () => {
    setShowAnswers(!showAnswers);  // Toggle visibility of answer explanations
  };
  return (
    ...
    {!quizStarted ? (
      <div className="user-details-section">
        ...
        <form onSubmit={handleSubmitDetails}>
          ...
          <button type="submit" className="btn btn-info">Start Quiz</button>
          </form>
      </div>
    ) : showResult ? (
      <div className="score-section">
        ...
        {/* Button to toggle answers section */}
        <button onClick={toggleViewAnswers} className="btn btn-success">
        ...
        {showAnswers && (
          ...
          <button onClick={restartQuiz} className="btn btn-primary pl-2">Restart Quiz</button>
        <button onClick={hideanswer} className="btn btn-outline-warning ml-2">HideAnswer</button>
      </div>
    ) : (
      <div className="question-section">
        <h1>Basic Computer Assessment</h1>
